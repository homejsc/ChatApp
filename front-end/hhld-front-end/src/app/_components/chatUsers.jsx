...
const ChatUsers = () => {
  ...
  useEffect(() => {
    ...
    const getMsgs = async () => {
      const result = await axios(`http://localhost:8080/msgs`, {
        params: {
          sender: authname,
        ...
        return (
          color.user === authname ? null : (
            ...
            <div key={index} onClick={() => setChatReceiver(color.user, index)} className={`${color.colour} rounded-full m-3 p-5`}>
              <span>{color.user} </span>  {(receivedMsgs.filter(msg => msg.sender === color.user).length > 0 ? <span> <span className='p-5 text-green-500'> {receivedMsgs.filter(msg => msg.sender === color.user).map(msg=> msg.msg).slice(length-1)} </span> <span className='p-5 text-black bg-white rounded-xl'> {receivedMsgs.filter(msg=> msg.sender === color.user).legnth} </span> </span> : null)}
            </div>)
        )
      }) : null}
    </div>
